...
function LoginPage() {
  ...
  // --- Registrar inicio de sesión en login_history ---
  const registerLoginHistory = async (user, providerLabel) => {
    const now = new Date();
    const loginAtMs = now.getTime();
    ...
    const docRef = await addDoc(collection(db, "login_history"), {
      ...
      loginDate: formatFechaYY(now),
      loginTime: formatHora(now),
      loginAtMs,
      logoutTime: null,
      logoutAtMs: null,
    ...
    // Guardamos para usar en el logout
    localStorage.setItem("currentLoginHistoryId", docRef.id);
    localStorage.setItem("sessionStartMs", String(loginAtMs));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await registerLoginHistory(userCred.user, "Email");
  ...
  const handleGoogleLinkLogin = async () => {
    try {
      ...
      // Verificar si el email existe en Firestore (colección usuario)
      ...
      if (querySnapshot.empty) {
        ...
        return Swal.fire(
          "Error",
          "El correo de Google no está registrado en nuestra base de datos.",
          "error"
        );
      }

      // 🔹 Registrar login en login_history (Google)
      await registerLoginHistory(user, "Google");
  ...
  const handleGithubLinkLogin = async () => {
    try {
      ...
      if (!email) {
        ...
        return Swal.fire(
          "Error",
          "No se pudo obtener el correo de GitHub.",
          "error"
        );
      }
      ...
      if (querySnapshot.empty) {
        ...
        return Swal.fire(
          "Error",
          "El correo de GitHub no está registrado en nuestra base de datos.",
          "error"
        );
      }

      // 🔹 Registrar login en login_history (GitHub)
      await registerLoginHistory(user, "GitHub");
      ...
      if (error.code === "auth/account-exists-with-different-credential") {
        ...
        if (!email) {
          return Swal.fire(
            "Error",
            "No se pudo obtener el correo del proveedor.",
            "error"
          );
        }
        ...
        if (methods.includes("password")) {
          ...
          try {
            ...
            Swal.fire(
              "Éxito",
              "Tu cuenta GitHub fue vinculada correctamente.",
              "success"
            );
            navigate("/DashboardPage");
          } catch (linkError) {
        ...
        {!showSuccess ? (
          <div ref={formRef}>
            ...
            <div className="social-login">
              <button
                type="button"
                className="social-btn neu-social"
                onClick={handleGoogleLinkLogin}
              >
