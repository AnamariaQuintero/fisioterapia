...
import {linkWithPopup, GoogleAuthProvider, GithubAuthProvider, signOut, fetchSignInMethodsForEmail, signInWithPopup, signInWithEmailAndPassword, linkWithCredential } from "firebase/auth";
...
// --- Método para login/vincular con Google ---
const handleGoogleLinkLogin = async () => {
  try {
    // 1️⃣ Abrir popup de Google
    const result = await signInWithPopup(auth, GoogleProvider);
    const user = result.user;
    const email = user.email.toLowerCase();

    // 2️⃣ Verificar si el email existe en Firestore
    const q = query(collection(db, "usuario"), where("email", "==", email));
    const querySnapshot = await getDocs(q);
    if (querySnapshot.empty) {
      await auth.signOut(); // cerrar sesión porque no existe en nuestra BD
      return Swal.fire("Error", "El correo de Google no está registrado en nuestra base de datos.", "error");
    }

    // 3️⃣ Registrar login en login_history
    const now = new Date();
    await addDoc(collection(db, "login_history"), {
      uid: user.uid,
      email: user.email,
    ...
    Swal.fire("Éxito", `Bienvenido, ${user.displayName || "Usuario"}`, "success");
    navigate("/DashboardPage");
...
// --- Método para login/vincular con GitHub ---
const handleGithubLinkLogin = async () => {
  try {
    ...
    if (!email) {
      await auth.signOut();
      return Swal.fire("Error", "No se pudo obtener el correo de GitHub.", "error");
    }
    ...
    if (querySnapshot.empty) {
      await auth.signOut();
      return Swal.fire("Error", "El correo de GitHub no está registrado en nuestra base de datos.", "error");
    }

    // Registrar login en login_history
    const now = new Date();
    await addDoc(collection(db, "login_history"), {
      uid: user.uid,
      email: user.email,
    ...
    // ✅ Capturamos el caso donde el correo ya existe con otro método
    if (error.code === "auth/account-exists-with-different-credential") {
      const pendingCred = GithubAuthProvider.credentialFromError(error);
      const email = error.customData?.email;

      if (!email) {
        return Swal.fire("Error", "No se pudo obtener el correo del proveedor.", "error");
      }

      // Verificamos qué métodos tiene ese correo
      const methods = await fetchSignInMethodsForEmail(auth, email);

      // Si existe una cuenta de contraseña normal
      if (methods.includes("password")) {
        const { value: password } = await Swal.fire({
          title: "Cuenta existente",
          input: "password",
          inputLabel: `El correo ${email} ya está registrado. Ingresa la contraseña para vincular GitHub.`,
        ...
        try {
          ...
          Swal.fire("Éxito", "Tu cuenta GitHub fue vinculada correctamente.", "success");
          navigate("/DashboardPage");
        } catch (linkError) {
          console.error("Error vinculando cuenta:", linkError);
          Swal.fire("Error", linkError.message, "error");
        }
      } else {
        Swal.fire(
          "Error",
          "Esta cuenta ya está vinculada con otro método (por ejemplo, Google). Intenta iniciar sesión con ese proveedor.",
          "error"
        );
      }
    } else {
        ...
        {!showSuccess ? (
          <div ref={formRef}>
            <form className="login-form" id="loginForm" noValidate onSubmit={handleSubmit}>
              ...
              <div className="form-group">
                <div className="input-group neu-input password-group">
                  ...
                  >
                    <svg style={{ flexShrink: 0}} className="eye-open" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" />
                      <circle cx="12" cy="12" r="3" />
                    </svg>
                    <svg className="eye-closed" style={{ flexShrink: 0}} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            ...
            <div className="vincular-wrapper">
              <Link to="/Login" className="vincular-link">Vincular proveedores</Link>
            </div>
            ...
            <div className="social-login">
              <button type="button" className="social-btn neu-social" onClick={handleGoogleLinkLogin}>
              ...
              <button type="button" className="social-btn neu-social" onClick={handleGithubLinkLogin}>
                <svg viewBox="0 0 24 24" fill="currentColor">
